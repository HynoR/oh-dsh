...
test('Web Docker image packages this checkout through the dist:web scripts', () => {
  ...
  assert.match(dockerfile, /libatomic1/)
  assert.doesNotMatch(dockerfile, /socat/)
  ...
  assert.doesNotMatch(entrypoint, /socat/)
  assert.match(entrypoint, /chown ohdsh:ohdsh \/data \/workspace/)
  assert.match(entrypoint, /\/usr\/bin\/setpriv --reuid=ohdsh --regid=ohdsh --init-groups/)
  assert.match(entrypoint, /--host 127\.0\.0\.1/)
  assert.match(entrypoint, /--port 3080/)
  ...
  const stage = readFileSync(join(root, 'scripts', 'stage-dsh.mjs'), 'utf8')
  assert.match(stage, /if \(!existsSync\(spec\.manifest\)\)/)
